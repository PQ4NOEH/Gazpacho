...
describe('Session goals', () => {
  ...
  it('Should add new goals', async () => {
    ...
    const handleOnChange = jest.fn();
    render(<SessionGoals goals={initialData} onChange={handleOnChange} />);
    fireEvent.input(screen.getByRole('textbox', { name: 'new goal text' }), {
      target: {
        value: 'A new goal',
      },
    });
  ...
  it('Should take away goals', async () => {
    const initialData = ['first goal', 'second goal', 'third goal'];
    const handleOnChange = jest.fn();
    render(<SessionGoals goals={initialData} onChange={handleOnChange} />);
    ...
    expect(goals).toEqual(['first goal', 'third goal']);
  });
